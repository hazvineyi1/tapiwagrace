...
- **The companion degrades, it does not break.** With no `ANTHROPIC_API_KEY`, or with one the API rejects, the endpoint returns 503 and the UI silently runs the scripted reflection instead. Never let a key problem surface an error to a visitor.
- **The key is checked at boot.** `verifyCompanionKey` lists models, the cheapest authenticated call there is, and the log says `accepted`, `no ANTHROPIC_API_KEY set`, or `REJECTED`. A set but invalid key looks exactly like a working one until someone tries to reflect, which is what this exists to catch.
- **Scripture is chosen by reference, never written by the model.** `src/lib/scripture.ts` holds 69 verified passages with theme tags; the model returns a reference and the server substitutes the text. A reference outside the library is dropped and logged. This is why the companion cannot misquote the Bible: regenerate with `scripts/build-scripture.py`, never by hand.
- **The reflection is short on purpose.** Three or four exchanges, then it closes. The server forces `closing` once the companion has spoken four times, except on a `care` turn, where it stays open so a woman in crisis is not shut out.
