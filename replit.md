...
- **Payment details are config, and start empty.** `src/lib/payments.ts` holds `STRIPE_DEPOSIT_LINK` and `BANK_TRANSFER`, both null until real values exist. Until then the page says a secure link is sent on confirmation, and no button is rendered. Never put a placeholder URL on a page that asks for money.
