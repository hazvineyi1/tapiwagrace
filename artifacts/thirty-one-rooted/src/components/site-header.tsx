...
export function SiteHeader() {
  ...
  const navLink = 'text-[13px] md:text-[10px] tracking-[0.2em] uppercase text-fg hover:text-rust transition-colors';
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 h-24 px-6 md:px-12 flex items-center justify-between z-50 transition-all duration-300 ${scrolled || !onHome ? 'bg-bg/95 backdrop-blur-md border-b border-line' : 'bg-transparent'}`}>
      ...
      <nav className={`fixed inset-0 bg-bg flex flex-col justify-center items-center gap-10 transition-transform duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] md:static md:bg-transparent md:flex-row md:justify-end md:gap-8 md:transform-none ${menuOpen ? 'translate-y-0' : '-translate-y-full md:translate-y-0'}`} aria-label="Main navigation">
        ...
        <button className="mt-8 md:mt-0 text-[11px] md:text-[10px] tracking-[0.2em] uppercase bg-moss text-bg px-8 md:px-6 py-4 md:py-3 hover:bg-fg transition-colors" onClick={() => { closeMenu(); openBooking(); }} data-testid="button-nav-book">Book a space</button>
      </nav>
