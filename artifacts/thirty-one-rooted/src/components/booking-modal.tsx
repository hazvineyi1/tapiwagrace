        ...
        {!complete ? (
          <>
            ...
            {step === 2 && (
              <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
                <label className="block">
                  <span className="text-[10px] tracking-[0.2em] uppercase text-fg/60 block mb-4">{isMeal ? 'Preferred delivery date' : 'Preferred date'}</span>
                  ...
                  <span className="mt-3 block text-[13px] text-fg/50 leading-relaxed">Dates are confirmed by reply — leave this blank if you are flexible.</span>
                </label>

                <div className="block mt-8">
                  <span className="text-[10px] tracking-[0.2em] uppercase text-fg/60 block mb-4">{isMeal ? 'Preferred delivery window' : 'Preferred time'}</span>
            ...
            {step === 3 && (
              <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
                ...
                <label className="block">
                  <span className="text-[10px] tracking-[0.2em] uppercase text-fg/60 block mb-4">{isMeal ? 'How many meals, and for whom?' : 'Anything we should know?'}</span>
                  <textarea value={message} onChange={(event) => setMessage(event.target.value)} rows={3} placeholder={isMeal ? 'Six meals a week for my mother, delivered in Borrowdale.' : 'Optional.'} className="w-full bg-transparent border-0 border-b border-line py-3 text-[16px] text-fg focus:ring-0 focus:border-moss transition-colors placeholder:text-fg/30 resize-none" data-testid="input-booking-message" />
                </label>
            ...
            <div className="flex items-center justify-between mt-12 pt-8 border-t border-line">
              {step > 1 ? (
                <button className="text-[10px] tracking-[0.2em] uppercase flex items-center gap-2 text-fg/50 hover:text-rust transition-colors" onClick={() => setStep(step - 1)} data-testid="button-booking-back">
